...
public class FunctionSet {
	...
	public static double EstTime(RandomAccessFile inputfn, int rounds) {
		...
		/*
		 * double estm=ests/60; //estimated minutes estm=estm*10000;
		 * estm=Math.round(estm); estm=estm/10000; //estminutes with only upto 5 decimal
		 * places
		 */
		return ests;
	}
	...
	public static String KeyGen(String key) // make a 200 byte key from user key
	{
		...
		for (int m = 0; m <= key.length() - 1; m++) {
			...
			System.out.println(
					"Generating 200 byte key from Entered key: " + FunctionSet.percentage(m, key.length() - 1) + "%");
		}
